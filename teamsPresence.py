...
def run():
    ...
    while True:
        ...
        try:
            result = requests.get(
                f'https://graph.microsoft.com/beta/me/presence', headers=headers, timeout=5)
            result.raise_for_status()
            ...
            if err.response.status_code == 404:
                ...
                trycount = trycount + 1
                printerror(
                    "MS Graph is not authorized. Please reauthorize the app (401). Try count: " + str(trycount))
                print()
                Authorize()
                continue
